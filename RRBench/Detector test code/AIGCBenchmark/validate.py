...
def validate_PSM(model, data_loader, opt):
    device = "cuda:2"
    y_true, y_pred = [], []
    i = 0
    ...
    with torch.no_grad():
        for data in data_loader:
            ...
            cropped_img = data[1].to(device)  # [batch_size, 3, 224, 224]
            label = data[2].to(device)  # [batch_size, 1]
            scale = data[3].to(device)  # [batch_size, 1, 2]
...
def validate(model, opt):
    ...
    if opt.detect_method == "Fusing":
        y_true, y_pred = validate_PSM(model, data_loader, opt)
    else:
        with torch.no_grad():
            for img, label in tqdm(data_loader, desc="Processing images"):
    ...
    y_true, y_pred = np.array(y_true), np.array(y_pred)
    r_acc = accuracy_score(y_true[y_true == 1], y_pred[y_true == 1] > 0.5)
    f_acc = accuracy_score(y_true[y_true == 0], y_pred[y_true == 0] > 0.5)
    acc = accuracy_score(y_true, y_pred > 0.5)
    ap = average_precision_score(y_true, y_pred)
