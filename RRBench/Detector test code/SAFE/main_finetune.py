...
def main(args):
    ...
    if not args.eval:
        ...
        dataset_val = None if args.disable_eval else TrainDataset(is_train=False, args=args)
    ...
    if args.eval:
        ...
        test_stats, acc, real_acc, fake_acc, ap = evaluate(
            data_loader_val, 
            model, 
            device, 
        ...
        print(f"Accuracy of the network on {len(dataset_val)} test images: {acc:.2%}")
        print(f"Real Accuracy: {real_acc:.2%}")
        print(f"Fake Accuracy: {fake_acc:.2%}")
        print(f"AP: {ap:.2%}")
        
        return test_stats, acc, real_acc, fake_acc, ap
